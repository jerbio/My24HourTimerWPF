...
namespace My24HourTimerWPF
{
    public class Schedule
    {
        ...
        */
        ...
        public Schedule(UserAccount AccountEntry)
        {
        ...
        private void Initialize()
        {
            ...
            AllEventDictionary = myAccount.getAllCalendarElements();
        ...
        {
            return myAccount.getAllCalendarElements();
        }
        ...
        public BusyTimeLine NextActivity
        {
            get
            {
                ...
                {
                    foreach (SubCalendarEvent MySubCalendarEvent in MyCalendarEvents.Value.AllActiveSubEvents)
                    {
        ...
        BusyTimeLine[] GetBusySlotPerCalendarEvent(CalendarEvent MyEvent)
        {
            ...
            {
                {*/
            foreach (SubCalendarEvent MySubCalendarEvent in MyEvent.AllActiveSubEvents)
            {
                if (!MyEvent.RepetitionStatus)
        ...
        public void deleteCalendarEvent(string EventID)
        {
            ...
            CalendarEventTOBeRemoved.DisableSubEvents(CalendarEventTOBeRemoved.AllActiveSubEvents);
            AddToSchedule(CalendarEventTOBeRemoved);
        ...
        public Tuple<CustomErrors, Dictionary<string, CalendarEvent>> SetEventAsNow(string EventID,bool Force=false)
        {
            ...
            CalendarEvent ScheduleUpdated = new CalendarEvent(ProcrastinateEvent.Name, StartTime.ToString("hh:mm tt"), StartTime, EndTime.ToString("hh:mm tt"), EndTime, AllValidSubCalEvents.Count.ToString(), ProcrastinateEvent.PreDeadline.ToString(), TotalActiveDuration.ToString(), new Repetition(), true, true, ProcrastinateEvent.Preparation.ToString(), true, ReferenceSubEvent.myLocation,true);
            SubCalendarEvent RigidizedEvent =ScheduleUpdated.AllActiveSubEvents[0];
            RigidizedEvent.shiftEvent(Now - RigidizedEvent.Start, Force);//remember to fix shift force option


            SubCalendarEvent RigidSubCalendarEvent = new SubCalendarEvent(RigidizedEvent.ID, RigidizedEvent.Start, RigidizedEvent.End, RigidizedEvent.ActiveSlot, RigidizedEvent.isEnabled, true, RigidizedEvent.myLocation, RigidizedEvent.getCalendarEventRange);
            ...
            SubCalendarEvent[] UpdatedSubCalevents = ScheduleUpdated.AllActiveSubEvents;
            ...
            for (int i = 0; i < AllValidSubCalEvents.Count; i++)//updates the subcalevents
            {
                ...
                SubCalendarEvent updatedSubCal = new SubCalendarEvent(AllValidSubCalEvents[i].ID, UpdatedSubCalevents[i].Start, UpdatedSubCalevents[i].End, UpdatedSubCalevents[i].ActiveSlot, Rigid, AllValidSubCalEvents[i].isEnabled, UpdatedSubCalevents[i].myLocation, ProcrastinateEvent.RangeTimeLine);
        ...
        public CalendarEvent GenerateRigidSubEvents(CalendarEvent MyCalendarEvent)
        {
            ...
            for (; i < MyCalendarEvent.AllActiveSubEvents.Length; i++)
            {
            ...
            if (MyCalendarEvent.RepetitionStatus)
            {
                ...
                SubCalendarEvent MySubEvent = new SubCalendarEvent(MyCalendarEvent.ActiveDuration, MyCalendarEvent.Repeat.Range.Start, MyCalendarEvent.Repeat.Range.End, MyCalendarEvent.Preparation, MyCalendarEvent.ID, MyCalendarEvent.Rigid,MyCalendarEvent.isEnabled, MyCalendarEvent.myLocation, MyCalendarEvent.RangeTimeLine);
        ...
        public CalendarEvent EvaluateTotalTimeLineAndAssignValidTimeSpots(CalendarEvent MyEvent, List<CalendarEvent> NoneCOmmitedCalendarEvent = null)
        {
            ...
            if (MyEvent.Rigid)
            {
                TempSubEvent = new SubCalendarEvent(MyEvent.ActiveDuration, MyEvent.Start, MyEvent.End, MyEvent.Preparation, MyEvent.ID, MyEvent.Rigid,MyEvent.isEnabled, MyEvent.myLocation, MyEvent.RangeTimeLine);
                MyTempBusyTimerLine = new BusyTimeLine(TempSubEvent.ID, TempSubEvent.Start, TempSubEvent.End);
                TempSubEvent = new SubCalendarEvent(TempSubEvent.ID, TempSubEvent.Start, TempSubEvent.End, MyTempBusyTimerLine, MyEvent.Rigid, TempSubEvent.isEnabled, MyEvent.myLocation, MyEvent.RangeTimeLine);
                        ...
                        if (MyCalendarEventUpdated != null)// && !MyCalendarEventUpdated.ErrorStatus)
                        {
                            ...
                            foreach (BusyTimeLine MyBusyTimeLine in TimeLineAndCalendarUpdated.Value.OccupiedSlots)
                            {
                                ...
                                if (ParentID != MyEventParentID)
                                {
                                    ...
                                    {
                                        MyArrayOfSubCalendarEvents = AllEventDictionary[ParentID].AllActiveSubEvents;
                                        for (i = 0; i < MyArrayOfSubCalendarEvents.Length; i++)
                                        {
        ...
        */
        ...
        {
            List<CalendarEvent> PertinentCalendarEvent = UncommitedCalendarEvents.Where(obj => obj.AllActiveSubEvents.Contains(possibleSubCalendarevent)).ToList();
        ...
        KeyValuePair<CalendarEvent, TimeLine> ReArrangeTimeLineWithinWithinCalendaEventRange(CalendarEvent MyCalendarEvent, List<CalendarEvent> NoneCommitedCalendarEventsEvents)// this looks at the timeline of the calendar event and then tries to rearrange all subevents within the range to suit final output. Such that there will be sufficient time space for each subevent
        {
            ...
            if (MyCalendarEvent.RepetitionStatus == false)//Artificially generates random subevents for the calendar event
            {
                ...
                for (i = 0; i < TempSubCalendarEventsForMyCalendarEvent.Length; i++)//populates the subevents for the calendar event
                {
                    ...
                    TempSubCalendarEventsForMyCalendarEvent[i] = new SubCalendarEvent(TempSubCalendarEventsForMyCalendarEvent[i].ID, (MyCalendarEvent.End - MyActiveTimeSpanPerSplit), TempSubCalendarEventsForMyCalendarEvent[i].End, new BusyTimeLine(TempSubCalendarEventsForMyCalendarEvent[i].ID, TempSubCalendarEventsForMyCalendarEvent[i].Start, TempSubCalendarEventsForMyCalendarEvent[i].End), MyCalendarEvent.Rigid, MyCalendarEvent.myLocation, MyCalendarEvent.RangeTimeLine);
                    MyCalendarEvent.updateSubEvent(TempSubCalendarEventsForMyCalendarEvent[i].SubEvent_ID, TempSubCalendarEventsForMyCalendarEvent[i]);
                }*/
            }
            else
            {
...
;
        ...
        Tuple<List<SubCalendarEvent>,TimeLine> every24Interval(List<SubCalendarEvent> currentListOfSubCalendarElements, TimeLine limitingTimeLine, double occupancy, Dictionary<string, mTuple<SubCalendarEvent, BusyTimeLine>> PreservedValues)
        {
            ...
            foreach (SubCalendarEvent eachmTuple in canExistWithinTimeLine.Concat(currentOccupiers))
            {
                TimeSpan ActiveTimeSpan = eachmTuple.ActiveDuration;
                string subcalStringID = eachmTuple.ID;
        ...
        */
            ...
            foreach (TimeLine MyTimeLine in MyListOfFreeTimelines)
            {
                ...
                foreach (CalendarEvent MyCalendarEvent in MyListOfPertitnentCalendars)
                {
                    ...
                    if (MyCalendarEvent.Repeat.Enable)
                    {
                        ...
                        {
                            SubCalendarEvent[] MyListOfSubCalendarEvents = MyRepeatCalendarEvent.AllActiveSubEvents;
        ...
        private SubCalendarEvent[] getInterferringSubEvents(TimeLine EventRange, List<CalendarEvent> NonCommitedCalendarEvemts = null)
        {
            ...
            foreach (KeyValuePair<string, CalendarEvent> MyCalendarEventDictionaryEntry in AllEventDictionary)
            {
                ...
                if (MyCalendarEventDictionaryEntry.Value.RepetitionStatus)
                {
                    ...
                    MyArrayOfInterferringSubCalendarEvents.AddRange(MyCalendarEventDictionaryEntry.Value.AllActiveRepeatSubCalendarEvents.Where(obj => obj != null).Where(obj => EventRange.InterferringTimeLine(obj.RangeTimeLine) != null).ToList());
                ...
                else
                {
                    ...
                    MyArrayOfInterferringSubCalendarEvents.AddRange(MyCalendarEventDictionaryEntry.Value.AllActiveSubEvents.Where(obj => obj != null).Where(obj => EventRange.InterferringTimeLine(obj.RangeTimeLine) != null).ToList());
            ...
            if (NonCommitedCalendarEvemts != null)
            {
                foreach (CalendarEvent eachCalendarEvent in NonCommitedCalendarEvemts)
                {
                    ...
                    if (eachCalendarEvent.RepetitionStatus)
                    {
                        ...
                        MyArrayOfInterferringSubCalendarEvents.AddRange(eachCalendarEvent.AllActiveRepeatSubCalendarEvents.Where(obj => obj != null).Where(obj => EventRange.InterferringTimeLine(obj.RangeTimeLine) != null).ToList());
                        /*for (i = 0; i < lengthOfCalendarSubEvent; i++)
                        {
                            //if ((EventRange.IsDateTimeWithin(ArrayOfSubcalendarEventsFromRepeatingEvents[i].Start)) || (EventRange.IsDateTimeWithin(ArrayOfSubcalendarEventsFromRepeatingEvents[i].End)))
                            if (EventRange.InterferringTimeLine(ArrayOfSubcalendarEventsFromRepeatingEvents[i].RangeTimeLine) != null)
                    ...
                    else
                    {
                        ...
                        MyArrayOfInterferringSubCalendarEvents.AddRange(eachCalendarEvent.AllActiveSubEvents.Where(obj => obj != null).Where(obj => EventRange.InterferringTimeLine(obj.RangeTimeLine) != null).ToList());
                        /*
                        for (i = 0; i < lengthOfCalendarSubEvent; i++)
                        {
        ...
        public Tuple<CustomErrors, Dictionary<string, CalendarEvent>> ProcrastinateJustAnEvent(string EventID, TimeSpan RangeOfPush)
        {
            ...
            List<SubCalendarEvent> AllValidSubCalEvents = ProcrastinateEvent.AllActiveSubEvents.Where(obj => obj.End > ReferenceSubEvent.Start).ToList();
            ...
            for (int i = 0; i < AllValidSubCalEvents.Count; i++)//updates the subcalevents
            {
                SubCalendarEvent updatedSubCal = new SubCalendarEvent(AllValidSubCalEvents[i].ID, UpdatedSubCalevents[i].Start, UpdatedSubCalevents[i].End, UpdatedSubCalevents[i].ActiveSlot, UpdatedSubCalevents[i].Rigid,AllValidSubCalEvents[i].isEnabled, UpdatedSubCalevents[i].myLocation, ProcrastinateEvent.RangeTimeLine);
                AllValidSubCalEvents[i].UpdateThis(updatedSubCal);
            }
