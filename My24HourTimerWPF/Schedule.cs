...
namespace My24HourTimerWPF
{
    public class Schedule
    {
        ...
        List<List<SubCalendarEvent>> SpreadOutEvents(List<List<SubCalendarEvent>> AlreadyAlignedEvents, Double AverageOccupiedSchedule, List<TimeLine> AllFreeSpots, Dictionary<TimeLine, Dictionary<string, Dictionary<string, mTuple<bool, SubCalendarEvent>>>> AllPossibleEvents, List<List<SubCalendarEvent>> RestrictedElements)
        {
            ...
            for (int i=0; i< AlreadyAlignedEvents.Count;i++)//Populates MovableElements dictionary
            {
                List<SubCalendarEvent> AlreadyAssigned = AlreadyAlignedEvents[i];
                List<SubCalendarEvent> RestrictedSubCalevent = RestrictedElements[i];
               ...
               IEnumerable< KeyValuePair<SubCalendarEvent , Tuple<int,TimeLine>>> ListOFeachKeyValuePair =MovableElements.Select(obj=>new KeyValuePair<SubCalendarEvent, Tuple<int,TimeLine>>(obj,new Tuple<int,TimeLine>(i, AllFreeSpots[i])));
                ...
                MovableElements_Dict.Add(AllFreeSpots[i], MovableForDict);
            }
                ...
                else
                {
                    if(Occupancy<AverageOccupiedSchedule)
                    {
                        ...
                        if(ExcessSpace.Ticks>0)
                        {
                            ...
                            ExcessSpace = TimeSpan.FromTicks((long)((1 - AverageOccupiedSchedule) * eachTimeLine.TimelineSpan.Ticks));
                            ...
                            List<mTuple<double, SubCalendarEvent>> removedElements = new List<mTuple<double,SubCalendarEvent>>();
                            ...
                            foreach(SubCalendarEvent eachSubCalendarEvent in NewlyAddedElements)
                            {
        ...
        public CalendarEvent EvaluateTotalTimeLineAndAssignValidTimeSpots(CalendarEvent MyEvent)
        {
            ...
            if (MyEvent.Rigid)
            {
                ...
                //SubCalendarEvent[] ClashingSubEvents = getInterferringSubEvents(MyEvent);
                ...
                KeyValuePair<CalendarEvent, TimeLine> TimeLineAndCalendarUpdated = ReArrangeClashingEventsofRigid(MyEvent);
        ...
        KeyValuePair<CalendarEvent, TimeLine> ReArrangeClashingEventsofRigid(CalendarEvent MyCalendarEvent)// this looks at the timeline of the calendar event and then tries to rearrange all subevents within the range to suit final output. Such that there will be sufficient time space for each subevent
        {
            /*
                Name{: Jerome Biotidara
            ...
            if (ArrayOfInterferringSubEvents.Length > 0)
            {
                ...
                List<SubCalendarEvent> collectionOfInterferringSubCalEvents = getInterferringSubEvents(RangeForScheduleUpdate).ToList();
            ...
            while (SumOfAllEventsTimeSpan >= RangeForScheduleUpdate.TimelineSpan)//loops untill the sum all the interferring events can possibly fit within the timeline. Essentially possibly fittable
            {
                ...
                List<SubCalendarEvent> collectionOfInterferringSubCalEvents = getInterferringSubEvents(RangeForScheduleUpdate).ToList();//updates interferring events list
            ...
            AllEventDictionary.Remove(MyCalendarEvent.ID);//hack alert global variable pain. 
...
;
                    ...
                    for (; FrontPartialCounter < ListOfFrontPartialsStartTime.Count; FrontPartialCounter++)
                    {
                        ...
                        else
                        {
                            ...
                            {//populates with final boundary for each restricted
                                ...
                                {
                                    EarliestReferenceTIme = RestrictedStopper;
                                }
            ...
            { //Handles THe Last Free Space outside of rigids
                ...
                if (ListOfFrontPartialsStartTime.Count > 0)
                {
                    for (FrontPartialCounter = 0; FrontPartialCounter < ListOfFrontPartialsStartTime.Count; FrontPartialCounter++)
                    {
                        ...
                        {
                            EarliestReferenceTIme = FreeBoundary.End;
                        }
                        foreach (SubCalendarEvent eachSubCalendarEvent in LowestCostArrangement)
                        {
                            --CompatibleWithList[eachSubCalendarEvent.ActiveDuration.Ticks.ToString()].Item1;
        ...
        List<mTuple<bool, SubCalendarEvent>> stitchRestrictedSubCalendarEvent(List<mTuple<bool, SubCalendarEvent>> Arg1, TimeLine RestrictingTimeLine)
        {
            ...
            TimeLine InterferringTimeLine = RestrictingTimeLine.InterferringTimeLine(AllSubCalEvents[0].getCalendarEventRange);
            ...
            foreach (SubCalendarEvent eachSubCalendarEvent in AllSubCalEvents)//gets the feasible timeLine foreach SubCalendarEvent
            {
                ...
                if ((InterferringTimeLine != null)&&(InterferringTimeLine.TimelineSpan>=eachSubCalendarEvent.ActiveDuration))
                {
                    ...
                    if ((CurrentRealignedTimeSpan <= SmallestAssignedTimeSpan) && (CurrentRealignedTimeSpan > ZeroTimeSpan))
                    {
                        if (AllSubCalEvents[i].getCalendarEventRange.End < AllSubCalEvents[indexOfSmallest].getCalendarEventRange.End)
                        {
                            indexOfSmallest = i;
        ...
        */
        ...
        private SubCalendarEvent[] getInterferringSubEvents(CalendarEvent MyCalendarEvent)
        {
            ...
            return getInterferringSubEvents(MyCalendarEvent.EventTimeLine);
        }


        private SubCalendarEvent[] getInterferringSubEvents(TimeLine EventRange)
        {
