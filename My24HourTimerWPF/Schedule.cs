    ...
    public class Schedule
    {
        ...
        async private Task Initialize(DateTimeOffset referenceNow)
        {
            ...
            //Tuple<Dictionary<string, CalendarEvent>, DateTimeOffset, Dictionary<string, Location_Elements>> profileData =await  myAccount.ScheduleData.getProfileInfo().ConfigureAwait(false);
            {
                ...
                AllEventDictionary = await myAccount.ScheduleData.getCalendarEvents().ConfigureAwait(false);// profileData.Item1;
                if (AllEventDictionary != null)
                {
                    //setAsComplete();
                    myCalendar.Add(ThirdPartyControl.CalendarTool.Outlook, new List<CalendarEvent>());
        ...
        public CalendarEvent getCalendarEvent(EventID myEventID)
        {
            ...
            CalendarEvent repeatEvent = calEvent.getRepeatedCalendarEvent(myEventID.getRepeatCalendarEventID());
        ...
        public Tuple<CustomErrors, Dictionary<string, CalendarEvent>> BundleChangeUpdate(string SubEventID, string NewName, DateTimeOffset SubeventStart, DateTimeOffset SubeventEnd, DateTimeOffset TimeLineStart, DateTimeOffset TimeLineEnd, int SplitCount)//, Time)
        {
            ...
            SubCalendarEvent ChangedSubCal = new SubCalendarEvent(mySubCalEvent.Id, SubeventStart, SubeventEnd, new BusyTimeLine(mySubCalEvent.Id, SubeventStart, SubeventEnd),mySubCalEvent.OrginalStartInfo, mySubCalEvent.Rigid, mySubCalEvent.isEnabled, mySubCalEvent.UIParam, mySubCalEvent.DataBlob, mySubCalEvent.isComplete,mySubCalEvent.RepetitionIndex, mySubCalEvent.Location, mySubCalEvent.getCalendarEventRange, mySubCalEvent.Conflicts);
        ...
        {
            CalendarEvent myCalendarEvent = getCalendarEvent(EventId);
            bool isNameChange = NewName != myCalendarEvent.NameString;
            bool isDeadlineChange = (newEnd) != myCalendarEvent.End;
            newStart = myCalendarEvent.Start;
        ...
        public CustomErrors deleteCalendarEventAndReadjust(string EventID)
        {
            ...
            CalendarEventTOBeRemoved = new CalendarEvent(CalendarEventTOBeRemoved.NameString, CalendarEventTOBeRemoved.Duration, CalendarEventTOBeRemoved.Start, CalendarEventTOBeRemoved.End, CalendarEventTOBeRemoved.OrginalStartInfo,CalendarEventTOBeRemoved.Preparation, CalendarEventTOBeRemoved.PreDeadline, CalendarEventTOBeRemoved.Rigid, new Repetition(), 1, CalendarEventTOBeRemoved.Location, false, new EventDisplay(), new MiscData(), false, -1);
            CalendarEventTOBeRemoved.DisableSubEvents(CalendarEventTOBeRemoved.ActiveSubEvents);
        ...
        async public Task markSubEventAsCompleteCalendarEventAndReadjust(string EventID)
        {
            ...
            {
                referenceCalendarEventWithSubEvent = referenceCalendarEventWithSubEvent.getRepeatedCalendarEvent(SubEventID.getRepeatCalendarEventID());
            }
            Dictionary<string, CalendarEvent> AllEventDictionary_Cpy = new Dictionary<string, CalendarEvent>();
            AllEventDictionary_Cpy = AllEventDictionary.ToDictionary(obj => obj.Key, obj => obj.Value.createCopy());
            ...
            CalendarEvent ScheduleUpdated = new CalendarEvent(referenceCalendarEventWithSubEvent.NameString, StartData, EndData, referenceCalendarEventWithSubEvent.OrginalStartInfo, 1.ToString(), referenceCalendarEventWithSubEvent.PreDeadline.ToString(), ReferenceSubEvent.ActiveDuration.ToString(), new Repetition(), true, ReferenceSubEvent.Rigid, referenceCalendarEventWithSubEvent.Preparation.ToString(), true, ReferenceSubEvent.Location, false, new EventDisplay(), new MiscData(), false);
        ...
        public async Task deleteSubCalendarEventAndReadjust(string EventID)
        {
            ...
            ReferenceSubEvent.delete(referenceCalendarEventWithSubEvent);
        ...
        /// </summary>
        /// <param name="EventID"></param>
        public async Task deleteSubCalendarEvent(string EventID)
        {
            CalendarEvent referenceCalendarEventWithSubEvent = getCalendarEvent(EventID);
            SubCalendarEvent ReferenceSubEvent = getSubCalendarEvent(EventID);
            ReferenceSubEvent.delete(referenceCalendarEventWithSubEvent);
        ...
        /// </summary>
        /// <param name="EventID"></param>
        public async Task deleteSubCalendarEvents(IEnumerable<string> EventIDs)
        {
        ...
        async public Task UpdateWithProcrastinateSchedule(Dictionary<string, CalendarEvent> UpdatedSchedule)
        {
        ...
        async public Task WriteFullScheduleToLogAndOutlook(CalendarEvent newEvent = null)
        {
            CleanUpForUI();
            ...
            await myAccount.CommitEvents(AllEventDictionary.ToDictionary(KeyValuePair=> KeyValuePair.Key, KeyValuePair => KeyValuePair.Value), newEvent);
        }
        ...
        List<List<SubCalendarEvent>> SpreadOutEvents(List<List<SubCalendarEvent>> AlreadyAlignedEvents, Double AverageOccupiedSchedule, List<TimeLine> AllFreeSpots, Dictionary<TimeLine, Dictionary<TimeSpan, Dictionary<string, mTuple<bool, SubCalendarEvent>>>> AllPossibleEvents, List<List<SubCalendarEvent>> RestrictedElements)
        {
            ...
            foreach (TimeLine eachTimeLine in AllFreeSpots)
            {
                ...
                if (Occupancy > AverageOccupiedSchedule)
                {
                    ...
                    List<mTuple<double, SubCalendarEvent>> MovableForDict = MovableElements.Select(obj => new mTuple<double, SubCalendarEvent>(DistanceSolver.AverageToAllNodes(obj.Location, MovableElements.Where(obj1 => obj1 != obj).ToList().Select(obj2 => obj2.Location).ToList()), obj)).ToList();
            ...
            foreach (TimeLine eachTimeLine in LessThanAverage.Select(obj => obj.Item1))
            {
                ...
                if (ExcessSpace.Ticks > 0)//tries to trim the UpdatedListForTimeLine. This is done by removing an element in the updated list until its detected that the origin timeline is below or equal to its average
                {
                    ...
                    List<mTuple<double, SubCalendarEvent>> NewlyAddedElementsWithCost = NewlyAddedElements.Select(obj => new mTuple<double, SubCalendarEvent>(DistanceSolver.AverageToAllNodes(obj.Location, UpdatedListForTimeLine.Select(obj3 => obj3.Item2).Where(obj1 => obj1 != obj).ToList().Select(obj2 => obj2.Location).ToList()), obj)).ToList();//creates mtuple of cost and subcal events
        ...
        Dictionary<CalendarEvent, List<SubCalendarEvent>> generateDictionaryWithBothCalendarEventIDAndListOfInterferringSubEvents(List<SubCalendarEvent> ListOfInterferringElements, List<CalendarEvent> UncommitedCalendarEvents)
        {
            ...
            for (; i < ListOfInterferringElements.Count; i++)
            {
                ...
                else
                {
                    ...
                    {
                        CalendarEvent repeatCalEvent = AllEventDictionary[ParentID].getRepeatedCalendarEvent(MyEventID.getRepeatCalendarEventID());
        ...
        Tuple<TimeLine, IEnumerable<SubCalendarEvent>, CustomErrors, IEnumerable<SubCalendarEvent>> getAllInterferringEventsAndTimeLineInCurrentEvaluation(CalendarEvent initializingCalendarEvent, List<CalendarEvent> NoneCommitedCalendarEventsEvents, int FlagType, HashSet<SubCalendarEvent> ExemptFromCalculation, TimeLine iniTimeLine)
        {
            ...
            List<SubCalendarEvent> SubEventsForCalculation = getAllElementsForCalculation(NonCommitedCalEvents.ToList(), RangeOfCalculation);
            TimeLine CalculationTImeLine = new TimeLine(Now.calculationNow, iniTimeLine.End);
            CustomErrors retCustomErrors = new CustomErrors(false, "No issue");
        ...
        /// <param name="IniTImeLine"></param>
        /// <returns></returns>
        List<SubCalendarEvent> getAllElementsForCalculation(List<CalendarEvent> InitializingCalEvents, TimeLine IniTImeLine)
        {
            /*
            ...
            {
                List<SubCalendarEvent> UnUsables = AllSubEventsThatCrossInitTimeLine.AsParallel().Where(obj => !obj.canExistWithinTimeLine(IniTImeLine)).ToList();
        ...
        */
        ...
        CalendarEvent ReArrangeTimeLineWithinWithinCalendaEventRangeUpdated(CalendarEvent MyCalendarEvent, List<CalendarEvent> NoneCommitedCalendarEventsEvents, int InterferringWithNowFlag, HashSet<SubCalendarEvent> NotDoneYet, bool OptimizeFirstTwentyFour = true, bool preserveFirstTwentyFourHours = true)// this looks at the timeline of the calendar event and then tries to rearrange all subevents within the range to suit final output. Such that there will be sufficient time space for each subevent
        {
            ...
            SortedInterFerringCalendarEvents_Deadline.ForEach(obj => obj.PrepareForCalculation());
        ...
        /*
        List<SubCalendarEvent> bestPathFirstTwentyFour(Location_Elements InitializingLocation)
        {
            TimeLine TwentyFourHoursTL = new TimeLine( Now.constNow,Now.constNow.AddDays(1));
            SubCalendarEvent[] AllInterferringSubevents = getInterferringSubEvents(TwentyFourHoursTL);
            List<Tuple<SubCalendarEvent, BusyTimeLine>> InitialLocation = AllInterferringSubevents.Select(obj => new Tuple<SubCalendarEvent, BusyTimeLine>(obj, obj.ActiveSlot.CreateCopy())).ToList();
            DateTimeOffset LatestTimeOfDay = AllInterferringSubevents.Max(obj => obj.End);
            TwentyFourHoursTL = new TimeLine(TwentyFourHoursTL.Start, LatestTimeOfDay > TwentyFourHoursTL.End ? LatestTimeOfDay : TwentyFourHoursTL.End);
            List<SubCalendarEvent> RigidSubcals = AllInterferringSubevents.Where(obj => obj.Rigid).ToList();
            TwentyFourHoursTL.AddBusySlots(RigidSubcals.Select(obj => obj.ActiveSlot));
            List<TimeLine> AllFreeSpots = TwentyFourHoursTL.getAllFreeSlots().ToList();

            //Continue from here Jerome
        ...
        */
        KeyValuePair<CalendarEvent, TimeLine> ReArrangeTimeLineWithinWithinCalendaEventRange(CalendarEvent MyCalendarEvent, List<CalendarEvent> NoneCommitedCalendarEventsEvents,int InterferringWithNowFlag, HashSet<SubCalendarEvent> NotDoneYet)// this looks at the timeline of the calendar event and then tries to rearrange all subevents within the range to suit final output. Such that there will be sufficient time space for each subevent
        {
            ...
            Tuple<TimeLine, IEnumerable<SubCalendarEvent>, CustomErrors, IEnumerable<SubCalendarEvent>> allInterferringSubCalEventsAndTimeLine = getAllInterferringEventsAndTimeLineInCurrentEvaluationOldDesign(MyCalendarEvent, NoneCommitedCalendarEventsEvents, InterferringWithNowFlag, NotDoneYet);
            List<SubCalendarEvent> collectionOfInterferringSubCalEvents = allInterferringSubCalEventsAndTimeLine.Item2.ToList();
            List<SubCalendarEvent> ArrayOfInterferringSubEvents = allInterferringSubCalEventsAndTimeLine.Item2.ToList();
            TimeLine RangeForScheduleUpdate = allInterferringSubCalEventsAndTimeLine.Item1;
            IEnumerable<SubCalendarEvent> NowEvents = allInterferringSubCalEventsAndTimeLine.Item4.Where(obj=>!obj.Rigid).ToList();
            ...
            //List<List<List<SubCalendarEvent>>> SnugListOfPossibleSubCalendarEventsClumps = BuildAllPossibleSnugLists(SortedInterFerringCalendarEvents_Deadline, MyCalendarEvent, DictionaryWithBothCalendarEventIDAndListOfInterferringSubEvents, ReferenceTimeLine, OccupancyOfTimeLineSPan);
            List<SubCalendarEvent> SnugListOfPossibleSubCalendarEventsClumps = BuildAllPossibleSnugLists( DictionaryWithBothCalendarEventIDAndListOfInterferringSubEvents.SelectMany(obj=>obj.Value).ToList(), ReferenceTimeLine, OccupancyOfTimeLineSPan,new List<SubCalendarEvent>());
            //Remember Jerome, I need to implement a functionality that permutates through the various options of pin to start option. So take for example two different event timeline that are pertinent to a free spot however one has a dead line preceeding the other, there will be a pin to start for two scenarios, one for each calendar event in which either of them gets pinned first.

            List<SubCalendarEvent> SerializedResult = SnugListOfPossibleSubCalendarEventsClumps.Except(preppedDataForNExtStage.Item2.SelectMany(obj => obj.getSubCalendarEventsInBlob())).ToList();  
        ...
        Dictionary<string, double> BuildDictionaryDistanceEdge(TimeLine ReferenceTimeline, CalendarEvent ReferenceCalendarEvent, Dictionary<string, double> CurrentDictionary)
        {
            ...
            for (i = 0; i < (ListOfBusySlots.Length - 1); i++)
            {
                ...
                {
                    double Distance = Location_Elements.calculateDistance(MyPrecedingCalendarEvent.Location, MyNextCalendarEvent.Location);
        ...
        */
        ...
        ulong ParallelizeCallsToDay(List<CalendarEvent> AllCalEvents,List<SubCalendarEvent> TotalActiveEvents, DayTimeLine[] AllDayTImeLine ,bool Optimize=true, bool preserveFirttwentyFourHours=true)
        {
            ...
            Dictionary<string, SubCalendarEvent> DictOfSubEvents = TotalActiveEvents.ToDictionary(obj => obj.Id, obj => obj);
        ...
        ILookup<ulong, SubCalendarEvent> PrepFirstTwentyFOurHours(List<CalendarEvent> AllCalEvents, TimeLine FirstTwentyFour)
        {
            ...
            ListOfTuples = ListOfTuples.Where(obj => ((obj.Item1.Rigid)||(new TimeLine(obj.Item2.End, obj.Item1.End).TimelineSpan < obj.Item1.EachSplitTimeSpan))).ToList();//gets elements that have to exist within timeframe, checks if span per fit is less than the intersecting span greater than first forty eight
        ...
        List<SubCalendarEvent> processTwentyFourHours(DayTimeLine myDayTimeLine, List<SubCalendarEvent> AllSubEvents)//,List<BusyTimeLine>BusySlots)
        {
            ...
            Location_Elements AvgLocation = Location_Elements.AverageGPSLocation((AllRigids.Concat(myDayTimeLine.getSubEventsInDayTimeLine())).Select(obj => obj.Location));
            ...
            Movables = Movables.OrderBy(obj => obj.EvaluatedScore).ThenBy(obj=>obj.fittability).ToList();
        ...
        {
            Dictionary<SubCalendarEvent, TimeLine> SUbEventTOInterferringTimeLine = AllSubEvents.ToDictionary(obj => obj, obj => new TimeLine(DayTimeLine.Start > obj.ProcrastinationInfo.PreferredStartTime ? DayTimeLine.Start : obj.ProcrastinationInfo.PreferredStartTime, obj.getCalendarEventRange.End));//selects the relevenat timeline per subeevent. Tries to use the prcrastination info to provide better preferenceing
            Dictionary<SubCalendarEvent, double> Distances = AllSubEvents.ToDictionary(obj => obj, obj => Location_Elements.calculateDistance(rigidLocation,obj.Location));
        ...
        List<SubCalendarEvent> BuildAllPossibleSnugLists(List<SubCalendarEvent> DictionaryWithBothCalendarEventsAndListOfInterferringSubEvents, TimeLine ReferenceTimeLine, double Occupancy, List<SubCalendarEvent> AlreadyAssigneEvents)
        {
            ...
            //Dictionary<TimeLine, List<SubCalendarEvent>> Dict_ConstrainedElements = (AlreadyAssigneEvents.Count > 0) ? JustPickCollidingTimeline(JustFreeSpots.ToList(), new HashSet<SubCalendarEvent>(AlreadyAssigneEvents).ToList()) : generateConstrainedList(JustFreeSpots.ToList(), AllInterferringSubEvents_Cpy);
        ...
        List<SubCalendarEvent> OptimizeTwentyFourHours(TimeLine myDay,List<SubCalendarEvent> currentConstituents , List<SubCalendarEvent> CanExistWInthinTImeLine_DoesNotIncludeCurrentConstituents, double averageOccupancy = 1)
        {
            ...
            */
            ...
            while (true)
            {
                ...
                ValidForCalculation=ValidForCalculation.OrderBy(obj => obj.EvaluatedScore).ToList();
        ...
        Tuple<List<SubCalendarEvent>,TimeLine> every24Interval(List<SubCalendarEvent> currentListOfSubCalendarElements, TimeLine limitingTimeLine, double occupancy, Dictionary<string, mTuple<SubCalendarEvent, BusyTimeLine>> PreservedValues)
        {
            ...
            { 
                ...
                {
                    CalEventDictionaryMapping.Add(calLevelID, new Tuple<Location_Elements, List<SubCalendarEvent>>(eachmTuple.Location, new List<SubCalendarEvent>() { eachmTuple }));
                }
            }

            

            Location_Elements AverageCurrentOccupiersGPSLocation= Location_Elements.AverageGPSLocation(currentOccupiers.Select(obj=>obj.Location));
            ...
            foreach (List<SubCalendarEvent> eachList in viableCombinations)
            {
                ...
                if ((eachList.Count < 1))
                {
                    ...
                    foreach (SubCalendarEvent eachSubCalendarEvent in MyList)
                    {
                        ...
                        { AllLocations.Add(eachSubCalendarEvent.Location); }
                    }
        ...
        */
        public Tuple<CustomErrors, Dictionary<string, CalendarEvent>> ProcrastinateJustAnEvent(string EventID, TimeSpan RangeOfPush)
        {
            ...
            for (int i = 0; i < AllValidSubCalEvents.Count; i++)//updates the subcalevents
            {
                ...
                SubCalendarEvent updatedSubCal = new SubCalendarEvent(AllValidSubCalEvents[i].Id, UpdatedSubCalevents[i].Start, UpdatedSubCalevents[i].End, UpdatedSubCalevents[i].ActiveSlot,UpdatedSubCalevents[i].OrginalStartInfo, UpdatedSubCalevents[i].Rigid, AllValidSubCalEvents[i].isEnabled, AllValidSubCalEvents[i].UIParam, AllValidSubCalEvents[i].DataBlob, AllValidSubCalEvents[i].isComplete,i, UpdatedSubCalevents[i].Location, ProcrastinateEvent.RangeTimeLine);
