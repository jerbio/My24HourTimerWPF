...
namespace TilerTests
{
    ...
    public class PathOptimization
    {
        ...
        public void PathOptimizationTIghtScheduleNoUnnecessaryConflict()
        {
            ...
            CalendarEvent testHomeNonRigidCalEvent = TestUtility.generateCalendarEvent(TimeSpan.FromHours(nonRigidHoursSpan), new TilerElements.Repetition(), encompassingTimeline.Start, encompassingTimeline.End.AddHours(1), 1, false, homeLocation);
            CalendarEvent testWorkNonRigidCalEvent = TestUtility.generateCalendarEvent(TimeSpan.FromHours(nonRigidHoursSpan), new TilerElements.Repetition(), encompassingTimeline.Start, encompassingTimeline.End.AddHours(1), 1, false, workLocation);
            int nonRigidTwoHoursSpan = 2;
            CalendarEvent testGymNonRigidCalEvent = TestUtility.generateCalendarEvent(TimeSpan.FromHours(nonRigidTwoHoursSpan), new TilerElements.Repetition(), encompassingTimeline.Start, encompassingTimeline.End.AddHours(1), 1, false, gymLocation);

            schedule.AddToSchedule(testHomeRigidCalEvent);
            schedule.AddToSchedule(testHomeNonRigidCalEvent);
            schedule.AddToSchedule(testWorkNonRigidCalEvent);
