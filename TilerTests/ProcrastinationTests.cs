...
namespace TilerTests
{
    ...
    public class ProcrastinationTests
    {
        ...
        public void procrastinateSingleEventAroundMultipleEvents()
        {
            ...
            Schedule = new TestSchedule(user, refNow);
            CalendarEvent testEvent0 = TestUtility.generateCalendarEvent(duration0, new Repetition(), start0, end0, 2, false);
            ...
            Schedule = new TestSchedule(user, refNow);
            CalendarEvent testEvent1= TestUtility.generateCalendarEvent(duration1, new Repetition(), start1, end1, 2, false);
        ...
        public void scheduleModificationWithProcrastinateAll()
        {
            ...
            Assert.IsTrue(testEventCopy.ActiveSubEvents.First().Start >= latestTime);
            Schedule = new TestSchedule(user, refNow);
            CalendarEvent testEvent0 = TestUtility.generateCalendarEvent(duration, new Repetition(), start, end.AddDays(1), 2, false);
            Schedule.AddToScheduleAndCommit(testEvent0).Wait();
            Schedule = new TestSchedule(user, refNow);
