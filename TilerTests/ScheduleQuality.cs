...
namespace TilerTests
{
    ...
    public class ScheduleQuality
    {
        ...
        [TestMethod]
        public void conflicting_subevent_should_haveProfile()
        {
            var packet = TestUtility.CreatePacket();
            UserAccount user = packet.Account;
            TilerUser tilerUser = packet.User;
            ...
            DateTimeOffset iniRefNow = DateTimeOffset.UtcNow.removeSecondsAndMilliseconds();
            DateTimeOffset now = iniRefNow;

            DateTimeOffset rigidStart = now.AddHours(1);
            TimeSpan rigidDuration = TimeSpan.FromHours(1);
            DateTimeOffset rigidEnd = rigidStart.Add(rigidDuration);
            TestUtility.reloadTilerUser(ref user, ref tilerUser);
            TestSchedule schedule = new TestSchedule(user, now);
            CalendarEvent rigidCalendarEvent = TestUtility.generateCalendarEvent(tilerUser, rigidDuration, null, rigidStart, rigidEnd, rigidFlags: false);
            schedule.AddToScheduleAndCommit(rigidCalendarEvent);

            CalendarEvent rigidCalendarEvent_DB = TestUtility.getCalendarEventById(rigidCalendarEvent.Id, user);
            SubCalendarEvent rigidSubEvent = rigidCalendarEvent_DB.ActiveSubEvents.First();
            int rigidConflictLevel = rigidSubEvent.conflictLevel;
            Assert.AreEqual(0, rigidConflictLevel);



            DateTimeOffset nonRigidStart = now;
            TimeSpan nonRigidDuration = TimeSpan.FromHours(2);
            DateTimeOffset nonRigidEnd = nonRigidStart.AddHours(3);
            TestUtility.reloadTilerUser(ref user, ref tilerUser);
            schedule = new TestSchedule(user, now);
            CalendarEvent nonRigidCalendarEvent = TestUtility.generateCalendarEvent(tilerUser, nonRigidDuration, null, nonRigidStart, nonRigidEnd, rigidFlags: false);
            schedule.AddToScheduleAndCommit(nonRigidCalendarEvent);


            CalendarEvent nonRigidCalendarEvent_DB =  TestUtility.getCalendarEventById(nonRigidCalendarEvent.Id, user);
            SubCalendarEvent subEvent = nonRigidCalendarEvent_DB.ActiveSubEvents.First();
            int nonRigidconflictLevel = subEvent.conflictLevel;
            Assert.AreEqual(0, nonRigidconflictLevel);
