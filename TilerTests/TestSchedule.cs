...
namespace TilerTests
{
    internal class TestSchedule : DB_Schedule
    {
        ...
        public TestSchedule(UserAccount AccountEntry, DateTimeOffset referenceNow, DateTimeOffset startOfDay, uint LatestId = 0, HashSet<DataRetrivalOption> retrievalOptions = null, TimeLine rangeOfLookup = null, HashSet<string> calendarIds = null) : base(AccountEntry, referenceNow, startOfDay,  retrievalOptions, rangeOfLookup, calendarIds: calendarIds)
        {
        ...
        public TestSchedule(UserAccount AccountEntry, DateTimeOffset referenceNow, uint LatestId = 0, HashSet<DataRetrivalOption> retrievalOptions = null, TimeLine rangeOfLookup = null, HashSet<string> calendarIds = null) : base(AccountEntry, referenceNow, retrievalOptions, rangeOfLookup, calendarIds: calendarIds)
        {
            if (LatestId != 0)
            {
                EventID.Initialize(LatestId);
        ...
        async override protected Task Initialize(DateTimeOffset referenceNow, DateTimeOffset StartOfDay, HashSet<string> calendarIds = null)
        {
            ...
            Tuple<Dictionary<string, CalendarEvent>, DateTimeOffset, Dictionary<string, Location>, Analysis> profileData = await myAccount.ScheduleData.getProfileInfo(RangeOfLookup, _Now, retrievalOptions, calendarIds: calendarIds).ConfigureAwait(false);
